...
bool seq_planner_class::IsAgentsSufficientForAction(vector<string> agent_name,string agent_type, string action_name){
	cout<<"seq_planner_class::IsAgentsSufficientForAction"<<endl;
	cout<<"actions_name: "<<action_name<<endl;
	cout<<"agent_type: "<<agent_type<<endl;
	cout<<"agent_name: ";
	...
	if(agent_name.empty()!=true && agent_type=="")// if a specific agent (by name) can perform an action
	{
		...
		for(int j=0;j<action_Definition_List[action_number].agents.size();j++)
		{
			...
			for(int l=0;l<action_Definition_List[action_number].agents[j].size();l++)
			{
				...
				for(int k=0;k<agent_name.size();k++)
				{
					cout<<"705: "<<count<<action_Definition_List[action_number].agents[j][l]<<"-"<<agent_name[k]<<endl;
					if(action_Definition_List[action_number].agents[j][l]==agent_name[k])
					{
						count++;
						cout<<"706: "<<count<<endl;
			...
			if(count==agent_name.size() && count==action_Definition_List[action_number].agents[j].size())
			{
				temp=true;
				break;
			}
	...
	{
		cout<<FRED("Not Implemented, temp = true")<<endl;
		temp=true;
	}
	else
	{
